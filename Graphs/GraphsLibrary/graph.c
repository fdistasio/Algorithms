...
// Marco un certo vertice come visitato e inserisco l'arco con il padre nel grafo 'result'. Quindi i è padre di parent[i]
	...
	while(visitedN<g.N)
	{
		...
		if((parent[v]==-1)||(parent[v]==v))
		{
			...
			while(e!=NULL){
				...
				{
					// Ho gia' visitato in precedenza il vertice di destinazione dell'arco 'e': controllo se l'arco
					// e' un arco all'indietro oppure un arco tra i due vertici in un grafo non diretto
		...
		// La pila e' vuota, ma non ho terminato di visitare tutti i vertici: creo un nuovo albero con il
...
Graph dfs_rec(Graph g, int start, int* hasCycles, int* nConComp)
{
	...
	// Visito tutti i vertici del grafo, saltando quelli gia' visitati
	for(int i=0; i<g.N; i++)
	{
		// Controllo se il vertice e' stato o no gia' visitato da una precedente visita dfs
